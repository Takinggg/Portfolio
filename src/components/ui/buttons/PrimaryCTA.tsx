...
export const PrimaryCTA: React.FC<PrimaryCTAProps> = ({
  ...
  const baseClasses = clsx(
    'relative inline-flex items-center justify-center font-semibold rounded-xl transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2',
    {
      ...
      // Color variants
      'bg-gradient-to-r from-primary-500 to-secondary-500 text-white shadow-md hover:shadow-lg focus:ring-primary-500': variant === 'primary',
      'bg-white text-primary-600 border-2 border-primary-200 hover:border-primary-300 focus:ring-primary-500': variant === 'secondary',
  ...
  return (
    <motion.button
      ...
      tabIndex={0}
    >
      {/* Background overlay for hover effect on primary variant */}
      {variant === 'primary' && !disabled && (
        <motion.div
          className="absolute inset-0 bg-gradient-to-r from-secondary-500 to-primary-500 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          initial={false}
        />
      )}
