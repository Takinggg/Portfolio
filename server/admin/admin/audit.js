...
export class AdminAuditService {
    ...
    logAction(entry) {
      ...
      `);
            stmt.run(entry.admin_user, entry.action, entry.resource_type, entry.resource_id || null, entry.old_values ? JSON.stringify(entry.old_values) : null, entry.new_values ? JSON.stringify(entry.new_values) : null, entry.ip_address || null, entry.user_agent || null);
        }
        catch (error) {
    ...
    getLogs(options = {}) {
        try {
            const { limit = 50, offset = 0, admin_user, resource_type, action, start_date, end_date } = options;
    ...
    getStats() {
        try {
            const totalLogs = this.db.prepare('SELECT COUNT(*) as count FROM admin_audit_logs').get();
