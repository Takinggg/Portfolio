...
export class SlotEngine {
     ...
     */
    async getAvailableSlots(eventTypeId, startDate, // ISO date string
    ...
    userTimezone = 'UTC') {
        ...
        while (currentDate <= effectiveEnd) {
            const daySlots = this.generateDaySlots(currentDate, eventType, rules, exceptions, existingBookings, userTimezone);
    ...
    generateDaySlots(date, eventType, rules, exceptions, existingBookings, userTimezone) {
        ...
        let dayRules = rules.filter(rule => rule.day_of_week === dayOfWeek && rule.is_active);
        ...
        for (const slot of mergedSlots) {
            ...
            while (currentSlotTime.plus({ minutes: eventType.duration_minutes }) <= slotEnd) {
                ...
                const hasConflict = this.hasBookingConflict(currentSlotTime.toJSDate(), slotEndTime.toJSDate(), existingBookings, eventType);
                if (!hasConflict) {
                    // Check daily quota
    ...
    `).all(eventTypeId, startDate.toISODate(), endDate.toISODate());
    }
    getExistingBookings(eventTypeId, startDate, endDate, excludeBookingUuid) {
        let query = `
      ...
      WHERE event_type_id = ? 
        ...
        AND end_time > ?
    `;
        const params = [eventTypeId, endDate.toISO(), startDate.toISO()];
     ...
     */
    async isSlotAvailable(eventTypeId, startTime, endTime, excludeBookingUuid) {
        try {
            const eventType = this.getEventType(eventTypeId);
            if (!eventType)
                return false;
            ...
            if (slotDuration !== eventType.duration_minutes)
                return false;
            // Get available slots for the day
            const availableSlots = await this.getAvailableSlots(eventTypeId, start.startOf('day').toISO(), start.endOf('day').toISO(), 'UTC');
            // Check if the requested slot is in the available slots
            return availableSlots.some(slot => slot.startUTC === startTime && slot.endUTC === endTime);
        }
        catch (error) {
