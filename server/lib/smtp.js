...
export async function sendMail(opts){
  ...
  const reader = socket;
  function cmd(line){ return new Promise((res, rej)=>{ socket.write(line+"\r\n"); reader.once('data', (d)=>{ const t=d.toString(); if (/^[45]/.test(t)) rej(new Error(t)); else res(); }); }); }
  ...
  await new Promise((res, rej)=> reader.once('data', (d)=>/^[25]/.test(d.toString())?res(null):rej(new Error(d.toString()))));
  socket.end('QUIT\r\n');
}