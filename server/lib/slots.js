...
import { readJSON } from './fsdb.js';

export async function computeSlots({ fromISO, toISO, durationMin, tz = process.env.TZ_EUROPE || 'Europe/Paris' }){
  const open = await readJSON('availability.json', {});
  const exceptions = new Set((await readJSON('exceptions.json', [])).map(s=>s.slice(0,10)));
