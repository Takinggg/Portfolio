 ...
 */
export function initializeSchedulingSchema(db) {
  try {
    ...
    const schedulingTables = tables.filter((table) => 
    ...
    console.log(`📊 Scheduling tables created: ${schedulingTables.map((t) => t.name).join(', ')}`);
 ...
 */
export function getSchedulingStats(db) {
  try {
    const stats = {
