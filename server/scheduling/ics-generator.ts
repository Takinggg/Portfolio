...
import { Booking, EventType, Invitee } from './types.js';
...
export class ICSGenerator {
   ...
   */
  static generateICS(booking: Booking, eventType: EventType, invitee: Invitee): string {
    const startTime = DateTime.fromISO(booking.start_time);
    const endTime = DateTime.fromISO(booking.end_time);
    const now = DateTime.now();

    // Format dates for ICS (YYYYMMDDTHHMMSSZ)
    const formatForICS = (dt: DateTime) => dt.toFormat('yyyyMMdd\'T\'HHmmss\'Z\'');
   ...
   */
  private static generateDescription(eventType: EventType, invitee: Invitee): string {
    const lines = [
   ...
   */
  private static getLocationText(locationType: string): string {
    switch (locationType) {
      case 'visio':
   ...
   */
  static getICSHeaders(filename?: string): Record<string, string> {
   ...
   */
  static generateFilename(eventType: EventType, startTime: DateTime): string {
  ...
  static validateICS(icsContent: string): { valid: boolean; errors: string[] } {
    ...
    if (!icsContent.includes('END:VEVENT')) {
      errors.push('Missing VEVENT END');
    }
