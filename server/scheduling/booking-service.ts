...
export class BookingService {
  ...
  constructor(
    ...
    actionTokenTTLHours: number = 168 // 7 days default
  ) {
    this.db = db;
   ...
   */
  async rescheduleBooking(request: RescheduleRequest): Promise<ActionResponse> {
    try {
      // Verify token
   ...
   */
  async cancelBooking(request: CancelRequest): Promise<ActionResponse> {
    try {
      // Verify token
   ...
   * Token management
   */
  private generateActionToken(bookingUuid: string, action: 'reschedule' | 'cancel'): string {
    const payload = {
      bookingUuid,
      action,
  ...
  private verifyActionToken(token: string, expectedAction: string): any {
    try {
      const payload = jwt.verify(token, this.actionTokenSecret) as any;
   ...
   * URL generation
   */
  private generateICSUrl(bookingUuid: string): string {
    return `/api/scheduling/booking/${bookingUuid}/calendar.ics`;
  }

  private generateGoogleCalendarUrl(booking: Booking, eventType: EventType): string {
  ...
  private getBooking(bookingId: string): Booking | null {
    return this.db.prepare('SELECT * FROM bookings WHERE uuid = ?').get(bookingId) as Booking || null;
  }

  private getBookingDetails(bookingUuid: string): (Booking & { invitee: Invitee }) | null {
